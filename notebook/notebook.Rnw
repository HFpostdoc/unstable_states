...
Note, time series will need to be aggregated:
``https://stat.ethz.ch/pipermail/r-help/2010-April/234036.html''
...
<<>>=
  n <- 500
            ...
            rm.stocks <- c('CREG','ASIA','ALLY','PRFE','PWO','TENX')
            index <- index[index$Ticker%in%rm.stocks==FALSE,]
            stocks <- sort(sample(index$Ticker,n))
            data <- list()
...
<<>>=
  mu.dif <- unlist(lapply(data,function(x) mean(I(x$Open-x$Close))))
            hist(mu.dif)
            t.test(mu.dif)
            stocks[mu.dif==min(mu.dif)]
            mu.dif[mu.dif==min(mu.dif)]
            mu.dif[mu.dif==max(mu.dif)]
@ 

<<>>=
  i <- sample(1:length(stocks),1);j <- sample(1:length(stocks),1)
            max.close <- max(do.call(rbind,data[c(i,j)])$Close)
            index[index$Ticker%in%stocks[c(i,j)],]
            par(mfrow=c(1,3))
            plot(Close~Date,data=data[[i]],main=paste(stocks[c(i,j)],collapse='_'),ylim=c(0,max.close))
            points(Close~Date,data=data[[j]],col=2,cex=0.25,pch=19)
            plot(Close~Date,data=data[[i]],main=stocks[i],ylim=c(0,max.close))
            plot(Close~Date,data=data[[j]],main=stocks[j],ylim=c(0,max.close))
